...
//		}
	...
	public static List<Meld> getRunMelds(List<Tile> tileList) {
		List<Tile> tilesToCheck = new ArrayList<Tile>(tileList.stream().filter(p -> !(p instanceof Joker)).collect(Collectors.toList()));
		List<Joker> jokers = new ArrayList<Joker>(tileList.stream().filter(p -> (p instanceof Joker)).map(p -> (Joker) p).collect(Collectors.toList()));
    ...
    int count = 0;
				...
				if (i + 1 < tiles.size() && (isRunOn = currTile.isRunOn(nextTile))) {
					...
					if (jokerUsed < jokerCount && currTile.getColour() == nextTile.getColour()) {
						...
						if (diff == 3 && jokerCount == 2 && jokers.get(0).isRunOn(jokers.get(1))) {
							...
							boolean firstJokerOnCurrent = (jokers.get(0).getPossibleTiles().isEmpty()) ? true : (currTile.getRank().getValue() == (jokers.get(0).getPossibleTiles().get(0).getRank().getValue() - 1) && currTile.getColour() == (jokers.get(0).getPossibleTiles().get(0).getColour())) ?
											true : 
											false;
							boolean secondJokerOnCurrent =  (jokers.get(1).getPossibleTiles().isEmpty()) ? 	true : (currTile.getRank().getValue() == (jokers.get(1).getPossibleTiles().get(0).getRank().getValue() - 1) && currTile.getColour() == (jokers.get(1).getPossibleTiles().get(0).getColour())) ?
											true : 
											false;
							...
							if (firstJokerOnCurrent && secondJokerOnCurrent) {
								...
								if (jokers.get(other).getPossibleTiles().isEmpty() || jokers.get(other).getPossibleTiles().get(0).getValue() + 1 == nextTile.getValue()) {
									//Set it up 
							...
							boolean firstJokerOnCurrent = (jokers.size()== 0) ? false : (jokers.get(0).getPossibleTiles().isEmpty()) ? true : (currTile.getRank().getValue() == (jokers.get(0).getPossibleTiles().get(0).getRank().getValue() - 1) && currTile.getColour() == (jokers.get(0).getPossibleTiles().get(0).getColour()) && jokers.get(0).getValue() + 1 == nextTile.getValue()) ?
									true : 
									false;
							boolean secondJokerOnCurrent =  (jokers.size()== 1) ? false :(jokers.get(1).getPossibleTiles().isEmpty()) ? 	true : (currTile.getRank().getValue() == (jokers.get(1).getPossibleTiles().get(0).getRank().getValue() - 1) && currTile.getColour() == (jokers.get(1).getPossibleTiles().get(0).getColour()) && jokers.get(1).getValue() + 1 == nextTile.getValue() ) ?
											true : 
											false;
							//IF both can play on first card
							if (firstJokerOnCurrent || secondJokerOnCurrent) {
		...
		for (Meld m: meldList) {
			...
			for (Joker j : jokers) {
				...
				boolean jokerOnFirst = (jokers.size()== 0) ? false : (j.getPossibleTiles().isEmpty()) ? true : (currTile.getRank().getValue() == (jokers.get(0).getPossibleTiles().get(0).getRank().getValue() + 1) && currTile.getColour() == (jokers.get(0).getPossibleTiles().get(0).getColour()) && currTile.getValue() > 1) ?
						true : 
						false;
				boolean jokerOnLast =  (jokers.size()== 0) ? false :(j.getPossibleTiles().isEmpty()) ? 	true : (lastTile.getRank().getValue() == (jokers.get(1).getPossibleTiles().get(0).getRank().getValue() - 1) && lastTile.getColour() == (jokers.get(1).getPossibleTiles().get(0).getColour()) && lastTile.getValue() != 13) ?
								true : 
								false;
		...
		for (int i = 0; i < 13; i++) {
			for (int j = 0; j < 2; j++) {
				...
				for (Joker joker: jokers) {
					List<Tile> possibleTiles = joker.getPossibleTiles();
					if (collectedSets.get(i).get(j).size() < 4) {
						...
						if (possibleTiles.isEmpty())
							...
							for (Tile tileToCheck: possibleTiles) {
	...
	static public MeldType checkMeldType(List<Tile> newTiles) {
		...
		List<Tile> tilesToCheck = new ArrayList<Tile>(newTiles.stream().filter(p -> !(p instanceof Joker)).collect(Collectors.toList()));
		//
