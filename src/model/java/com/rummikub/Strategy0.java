...
public class Strategy0 implements StrategyBehaviour {
	...
	@Override
	public List<Meld> useStrategy(Player currentPlayer) {// Basic Variables Needed
		...
		if (currentPlayer.canPlayOnExistingMelds) {
			while (userIsPlaying) {
				choiceOfPlayS = Prompt.promptInput(
						"Choose 1 to play from hand, and 2 to play using the Table: (-1 to play noting)(0 to end turn)");
	...
	public void playStrategy(Player currentPlayer, List<Meld> tableMelds, List<Meld> returnMelds) {
		...
		inputIntegerList = Prompt.promptUserTiles(
				"Choose the tiles that you want to play on the melds you have chosen (Ex: 1 10 11 12)",
				currentPlayer.getPlayerRack());
	...
	public void initialStrategy(Player currentPlayer, List<Meld> possibleMelds, List<Meld> returnMelds) {
		...
		while (playerIsChoosing) {
			...
			// Add the melds chosen to returnMelds, removeTheTiles played, and update the
			// melds he can play.
	...
	public void initialStrategy2(Player currentPlayer, List<Meld> possibleMelds, List<Meld> returnMelds,
			List<Tile> toPlayWithRack) {
		boolean playerIsChoosing = true;
